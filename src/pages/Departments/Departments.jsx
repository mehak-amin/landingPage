    ...
    const fetchDepartments = async ()=>{
        try{
           const response = await axios({
            method:"GET",
            url:"http://tracker.ap-south-1.elasticbeanstalk.com/api/v1/departments",
        })
...
const handleDeleteDepartment = async ()=>{
  try{
    console.log(id)
      const response = await axios({
          method:"DELETE",
          url:`http://tracker.ap-south-1.elasticbeanstalk.com/api/v1/departments/${id}`
      })
      fetchDepartments();
      ...
      {/* <--------- DELETE POPUP STRUCTURE -----------> */}
{
            ...
            <div onClick={()=>(setDeletePopUp(!deletePopUp))}><h4><FontAwesomeIcon icon={faCircleXmark} /></h4></div>
          </div>

          <div className="delete-department-center-departments">
            <h6>Do you really want to remove the projects that you have chosen?</h6>
      ...
      <div className="create-department-popup-wrapper">
        ...
        <div className="create-department-top-departments">
          ...
          <div className="create-department-bottom-departments">
            <div className="create-department-cancle-bottom-departments" onClick={() => setDeptName('')}><h6>Cancel</h6></div>
            <div className="create-department-invite-bottom-departments" onClick={handleCreateDepartment}><h6>Create</h6></div>
          </div>
      </div>
      </div>
      }
            ...
            {departmentsData.map((department, departmentIndex) =>(
             ...
             <div key={departmentIndex} className="table-container-departments">
                <table>
                    ...
                    <tr>
                        <td className='table-data-appname-departments'><input type="checkbox" /> <h6>{department.department_name}</h6></td>
                        <td><h6>{department.created_at}</h6></td>
                        <td><h6>3</h6></td>
                        <td><h6 onClick={()=>{toggleEditOrDeletePopUp(department.id)
                                              setId(department.id);
                        }}>
